	...
	return Controller.extend("vikalp.cus.sd.dealer.plus.controller.S3", {
		...
		_handleRouteMatched: function(oEvent) {
			...
			if (_self._isObjectEmpty(loObject)) {
				...
				var loDisplayForm = _self._getDisplayForm();
...
//				var loTabBar = _self.getView().byId("idIconTabBar");
//				if (loTabBar.getSelectedKey() !== "General"){
...
//					this.getView().byId("cancel").setVisible(false);
//				}
				var loGeneralTab = this.getView().byId("idgeneraltab");
				loDisplayForm.bindElement(loHeaderPath);
				loGeneralTab.removeAllContent();
				loGeneralTab.insertContent(loDisplayForm);
				
				//Ship To Party table
				
				var loShipList = this.getView().byId("idShipTable");

				loShipList.bindItems({
					path: loHeaderPath + "/Shiptoparty",
					template: new sap.m.ColumnListItem({
//						type: "Navigation",
						cells: [

							new sap.m.Text({
								text: "{dealer>CustomerName} ({dealer>CustomerId})"
							}),
							new sap.m.Text({
								text: "{dealer>SalesOrganization},{dealer>DistributionChannel},{dealer>Division}"
							}),
							new sap.m.Text({
								text: "{dealer>Street}"
							}),
							new sap.m.Text({
								text: "{dealer>City},{dealer>Region},{dealer>RegionName},{dealer>CountryName}"
							}),
							new sap.m.Text({
								text: "{dealer>PostalCode}"
							}),
							new sap.m.Text({
								text: "{dealer>ContactNumber}"
							}),
							new sap.m.Text({
								text: "{dealer>MailId}"
							}),
...
//				}
		...
		/**
		 * To Display Ship to Party data 
		 * 
		 * @param [object] oEvent: Binding Path
		 */
		
		_getDisplayForm : function(){
			if (!this._oDisplayForm) {
				this._oDisplayForm = sap.ui.xmlfragment("vikalp.cus.sd.dealer.plus.view.GeneralDataDisplay", this);
				this.getView().addDependent(this._oDisplayForm);
				}
			
			return this._oDisplayForm ;
		},
...
//				var loUrl = locurrentURL + "/Contacts/$count";
//				loModel.read(loUrl,null, null, false, function(oData, oResponse){
//					loCount = oResponse.body;
//				}, function(){
//					console.log('e')
//				});
