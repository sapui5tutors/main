jQuery.sap.declare("testcamera.devapp");
jQuery.sap.require("testcamera.devlogon");

testcamera.devapp = {
	smpInfo: {},
	//the variable hold testcamera.devlogon instance
	devLogon: null,
		...
		if (window.sap_webide_FacadePreview) {
			...
			$.getJSON(".project.json", function(data) {
				...
				if (that.smpInfo.server && that.smpInfo.server.length > 0) {
					...
					that.devLogon = new testcamera.devlogon();
